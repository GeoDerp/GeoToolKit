...
def test_mcp_create_projects_validation(tmp_path):
    ...
    assert any("api.test.com:80" in h or "api.test.com:443" in h for h in hosts), f"Expected api.test.com with port in {hosts}"
...
def test_mcp_allowlist_derivation_edge_cases():
    ...
    assert any("github.com:80" in host for host in hosts), f"Should include github.com:80 in {hosts}"
