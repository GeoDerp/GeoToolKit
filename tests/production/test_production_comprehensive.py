...
class TestProductionReadiness:
        ...
        for lang, project_info in production_suite.test_projects.items():
            try:
                ...
                assert result["vulnerabilities_found"] > 0, (
                    f"No vulnerabilities found in {lang}"
                )
                logger.info(
                    f"✅ {lang} scan completed - {result['vulnerabilities_found']} issues found"
                )
    ...
    @pytest.mark.timeout(600)  # 10 minute timeout for concurrent load testing
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for i in range(8):  # 8 concurrent scans
                ...
                future = executor.submit(
                    production_suite._simulate_scan, project_info, f"load_test_{i}"
                )
    ...
    @pytest.mark.timeout(120)  # 2 minute timeout for container security testing
    ...
    @pytest.mark.timeout(60)  # 1 minute timeout for network isolation testing
    ...
    @pytest.mark.timeout(300)  # 5 minute timeout for report generation performance
    def test_report_generation_performance(self, production_suite):
        ...
        for test in report_tests:
            ...
            logger.info(
                f"✅ {test['name']} report: {generation_time:.2f}s, {len(report_content)} chars"
            )
...
"""
    ...
    @pytest.mark.timeout(180)  # 3 minute timeout for memory usage testing
    def test_memory_usage(self, production_suite):
        ...
        try:
            ...
            for _i in range(3):
                dataset = {
                    "findings": [{"data": f"finding_{j}" * 100} for j in range(1000)]
                }
            ...
            logger.info(
                f"✅ Memory test - Initial: {initial_memory:.1f}MB, Final: {final_memory:.1f}MB"
            )
