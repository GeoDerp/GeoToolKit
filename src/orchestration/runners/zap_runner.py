...
class ZapRunner:
    ...
    @staticmethod
    def run_scan(target_url: str, network_allowlist: list[str] | None = None) -> list[Finding]:
        ...
        try:
            try:
                ...
                print("Podman not found; attempting to connect to existing ZAP instance...")
            ...
            wrapped_json_output = json.dumps({
                "site": [{"@name": target_url, "alerts": alerts_response.json().get("alerts", [])}]
            })
            ...
            if container_started:
                try:
                    ...
                    subprocess.run(["podman", "stop", container_name], capture_output=True, timeout=10)
                    subprocess.run(["podman", "rm", "-f", container_name], capture_output=True, timeout=10)
                except Exception:
                    pass
        return findings
